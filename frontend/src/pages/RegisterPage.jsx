...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error instanceof yup.ValidationError) {
        ...
        setError({ general: 'Неверный логин/пароль' });
      }
    }
  };
  ...
  return (
    <div className="menu_main_reg">
      ...
      {error.email && <span className="error-message_2">{error.email}</span>}

      <InputReg placeHolder={"Пароль"} marginTop={"2%"} image={lock} input_type={"password"} value={password}
        onChange={(e) => setPassword(e.target.value)} />
      {error.password && <span className="error-message_2">{error.password}</span>}
      {error.general && <span className="error-message_2">{error.general}</span>}
